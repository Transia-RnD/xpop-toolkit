...
export async function fundSystem(
  ...
  if ((await limit(client, dave.wallet.classicAddress, USD)) < 20000) {
    needsLines.push(dave.wallet)
  }
  if ((await limit(client, elsa.wallet.classicAddress, USD)) < 20000) {
    needsLines.push(elsa.wallet)
  }
  // Check IC Balance
  const needsIC: string[] = []
  if ((await balance(client, alice.wallet.classicAddress, USD)) < 2000) {
    needsIC.push(alice.wallet.classicAddress)
  }
  if ((await balance(client, bob.wallet.classicAddress, USD)) < 2000) {
    needsIC.push(bob.wallet.classicAddress)
  }
  if ((await balance(client, carol.wallet.classicAddress, USD)) < 2000) {
    needsIC.push(carol.wallet.classicAddress)
  }
  if ((await balance(client, dave.wallet.classicAddress, USD)) < 2000) {
    needsIC.push(dave.wallet.classicAddress)
  }
  if ((await balance(client, elsa.wallet.classicAddress, USD)) < 2000) {
    needsIC.push(elsa.wallet.classicAddress)
  }

  await fund(client, wallet, new ICXRP(10000), ...needsFunding)
  await trust(client, USD.set(20000), ...needsLines)
